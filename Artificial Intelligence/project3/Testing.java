...
public class Testing extends MainProgram{
    ...
    public static int search(int Matrix[][]){
        ...
        for (int i = 0; i < nTraining; i++){
            int dist = 0;
            if(viewed2)
                dist = Euclid(Matrix, TrainingSample[i].image);
            else if(!viewed2)
                dist = Manhattan(Matrix, TrainingSample[i].image);
...
/*    public static double roundTwoDecimals(double d){
    ...
    public static void showResult(int index){
        ...
        if (res == ans){
            count++;
        //    rate += 100.0 / nTesting;
            Rate.setText("Recognition rate: " + Integer.toString(count) + "/"+ Integer.toString(nTesting));
        }
    ...
    public Testing() throws IOException {
        if (nTraining == 0){
            JOptionPane.showMessageDialog(MainFrame, "First train please!", "Notice", JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        viewed2 = (ViewBox2.getSelectedIndex() == 0);
